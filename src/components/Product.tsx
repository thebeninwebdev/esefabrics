...
import type { IProduct, CartItem, IVariationArray } from "@/app/types"
...
export default function Products({
  ...
  variations,
  wishlist,
}: { products: IProduct[]; variations: IVariationArray[]; wishlist?: boolean }) {
  const {
    setDrawerId,
    setIsDrawerOpen,
    cart,
  ...
  useEffect(() => {
    ...
    // If products are available, hide skeleton after a short delay
  ...
  if (isProductsLoading) {
    return <ProductsSkeleton count={products?.length || 8} />
  }

  return (
    <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 max-w-max mx-auto">
      {products?.map((product: IProduct, idx: number) => (
        ...
        >
          <div className="relative">
            ...
            {wishlist ? (
              <button
                className="absolute top-2 right-2 p-2 text-text-dark dark:text-text opacity-75 hover:opacity-100 bg-primary dark:bg-primary-dark rounded-full"
                onClick={() => {
                  setLoading(true)
              ...
              <div className="flex justify-between w-36 py-2 px-4 h-max text-2xl font-thin items-center text-text bg-neutral-300 rounded-md ">
                ...
                <span className="text-lg font-normal">
                  {cart
                    ...
                    .reduce(
                      (sum: number, item: CartItem) => sum + (item.quantity || 0) + (item?.variant?.quantity || 0),
                      0,
                    )}
                </span>
                <button
