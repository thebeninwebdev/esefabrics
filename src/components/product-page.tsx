...
import type { IProduct, VariationInterface, CartItem } from "@/app/types"
...
export default function ProductComponent({ resolvedParams }: any) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        await Promise.all([fetchProducts(), fetchVariations()])
      } catch (error) {
    ...
    // Force skeleton to show for at least 1.5 seconds for better UX
    const timer = setTimeout(() => {
      if (products?.length > 0) {
        ...
        if (product) {
          ...
          // If product not found after timeout
  ...
  useEffect(() => {
    // Update current product when products are loaded
  ...
  // Show skeleton while loading with an animation
  if (isProductLoading) {
    return (
      <>
  ...
  return (
    <>
      <div className="pt-10 max-w-7xl w-full mx-auto animate-fadeIn">
          ...
          <div className="md:grid grid-cols-2">
            ...
            <div className="overflow-y-auto">
              ...
              <div className="px-3 space-y-4">
                ...
                <div className="flex items-center gap-5">
                  ...
                  <span className="text-xs bg-primary dark:bg-primary-dark text-text-dark rounded-2xl px-3 py-1">
                    {Math.round(
                      ((currentProduct.retailPrice - currentProduct.discountedPrice) / currentProduct.retailPrice) *
                        100,
                    )}
                    % OFF
                  </span>
              ...
              <div className="py-5 px-3 space-y-5">
                {variations &&
                  variations?.find(
                    (variation: VariationInterface) => variation?.reference_id === currentProduct?._id,
                  ) && (
                    <div className="">
                      ...
                      <div className="flex flex-wrap gap-2 pt-3">
                        {variations &&
                          variations
                            ?.find((variation: VariationInterface) => variation?.reference_id === currentProduct?._id)
                            ?.variations?.map((variation: any, idx: number) => (
                              <div
                                key={idx}
                                onClick={() => setIsCartSelection(true)}
                                className="py-1 px-3 rounded-md border-primary border-2 dark:bg-primary-dark dark:text-text-dark text-text w-max"
                              >
                ...
                {cart.find((item: CartItem) => item._id === currentProduct._id) ? (
                  ...
                  <div className="flex gap-5">
                    ...
                    <div
                      onClick={() => addToWishlist(userId as string, currentProduct._id as string)}
        ...
        {currentProduct?.description && (
          <div
            className="prose dark:prose-invert w-full break-words whitespace-normal text-text dark:text-text-dark h-96 overflow-auto max-w-4xl mx-auto my-16 px-5"
            style={{
